...
OFFLINE_RECONCILIATION_PASS. HH hydration PASS and Application Card Full V4
preview/confirm wiring PASS. Private V4 now uses the accepted `SKIP => no
automatic cover letter` contract. Installed engine metadata: v4.0.1,
aggregate hash `b9aa478c53d68cc9c02783372ae7ed90e601fc45ab1302f8678417c2c0f24d78`.
No provider call was used for reconciliation. One live Full V4 acceptance for
the one authorized live acceptance could not reach the card: the real HH page
loaded, while the extension Side Panel received no vacancy context and Inbox
stayed empty. No execute/confirm action or provider call was made; this branch
is still unmerged.
...
| decision-dependent letter | reconciled private V4 + public invariant: SKIP has no letter | PASS | PASS | PASS |
| sections / evidence IDs | private source + public validators | PASS | PASS | PASS |

Private V4 was reconciled locally in its own repository; no private source is
...
- Card labels distinguish Stage A deterministic values from Full V4 score/decision/confidence and render the persisted execution result. SKIP explicitly shows that no letter was generated due to SKIP.
...
and its full vacancy response. Prior deterministic dogfood evidence confirms
hydration for `136022615` with sanitized description length 3461 and six key
skills; Preview Full V4 is provider-free. The single authorized live
execute/confirm run was not reached because the Side Panel context handoff
failed and no Application Card was available. No raw candidate evidence or
private prompt is exported.

## Tests

- HH client GET/ID validation and hydration/idempotency regressions: PASS.
- Companion format, Ruff, strict mypy, full 374-test pytest suite and OpenAPI drift: PASS.
...
- Reconciliation and the blocked live attempt made zero real provider calls;
  live acceptance is invalid because the card context was unavailable.

## Git

Branch: `hotfix/hh-vacancy-hydration-v4-card`.

Current hotfix HEAD: `bf9ab13c53ce316b30ce0e4fe43729a61c9fc2be`.
The branch remains intentionally unmerged and unpushed. Private V4 files remain
separate; the reconciled private commit is local-only and was not pushed.
