...
The runtime uses the official API base URL, a stable User-Agent, bounded timeouts, and sanitized error codes. The OAuth client secret is an OS-keyring secret; client id and the registered redirect URI are non-secret local configuration. No HH cookie, password, or session is handled.
