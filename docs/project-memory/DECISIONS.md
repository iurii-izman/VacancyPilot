...
| ADR-007 | ACCEPTED | Full V4 Preview is provider-free and has no application/status side effects, but may persist one official read-only HH hydration for an incomplete selected vacancy; Application Factory Preview remains fully side-effect-free. | Preserves the current useful workflow while keeping provider, application, status and HH-write boundaries explicit. | [`ADR-007`](../development/application-ops/adr/ADR-007-full-v4-preview-hydration.md) | — |
| ADR-008 | ACCEPTED | Provider execution is bound to a deterministic reviewed plan, authenticated receipt, current policy, semantic single-flight claim, atomic real-attempt budget, and recursive privacy-safe input. | Prevents Preview/Execute drift, duplicate dispatch, budget races, raw-output persistence and privacy widening across Standalone and Companion paths. | [`ADR-008`](../development/application-ops/adr/ADR-008-provider-execution-privacy-boundary.md) | — |
