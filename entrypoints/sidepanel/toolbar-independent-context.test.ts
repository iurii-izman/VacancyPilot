...
describe("toolbar-independent HH vacancy context", () => {
  ...
  it("resolves page kind through the live content script and validates vacancy URLs", () => {
    ...
    expect(background).toContain("isCanonicalHhVacancyReference");
  });

  it("binds the badge-opened tab by window and fails closed without live proof", () => {
    expect(background).toContain("sidePanelBindingStorageKey(windowId)");
    expect(background).not.toContain("contextMatchesTab(context, targetTabId, targetWindowId)");
    expect(background).toContain("await clearTabContext(targetTabId)");
