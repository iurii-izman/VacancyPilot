...
function FollowUpPanel({ item }: { item: WorkItemViewModel }): ReactNode {
  ...
  const clearStandaloneFollowUp = async () => {
    ...
    await db.applications.update(localApplicationId, { followUpAt: undefined });
...
export function ApplicationWorkspace({ onNavigate }: { onNavigate?: (route: "discovery") => void }): ReactNode {
  const [directVacancyId, setDirectVacancyId] = useState<string | null>(() => new URLSearchParams(window.location.search).get("vacancyId"));
  ...
  if (selected) return <ApplicationCard item={selected} onBack={() => { window.history.replaceState({}, "", `${window.location.pathname}#inbox`); setSelected(null); setDirectVacancyId(null); }} />;
  if (directVacancyId && total === 0) return <section aria-labelledby="inbox-title"><h2 id="inbox-title" style={pageTitle}>Inbox</h2><p role="alert">Application card vacancy was not found.</p><button type="button" onClick={() => setDirectVacancyId(null)}>Open Inbox</button></section>;
