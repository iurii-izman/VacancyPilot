...
export function HHIntegrationSection(): ReactNode {
  ...
  const sync = async () => { setBusy(true); setError(null); try { setSummary((await getOpsClient().syncHHVacancies({ all_enabled: true })).data); } catch { setError('HH vacancy sync failed.'); } finally { setBusy(false); } };
